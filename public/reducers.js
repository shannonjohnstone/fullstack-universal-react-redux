...
var _booksReducers = __webpack_require__(8);

var _booksReducers2 = _interopRequireDefault(_booksReducers);

var _cartReducers = __webpack_require__(9);
...
exports.default = function () {
  ...
  switch (type) {
    ...
    case 'FETCH_BOOKS_SUCCESS':
      {
        return _extends({}, state, {
          isSet: action.isSet,
          isFetching: action.isFetching,
          books: [].concat(_toConsumableArray(state.books), _toConsumableArray(action.books))
        });
      }
    ...
    case 'DELETE_BOOK':
      {
        var bookIndexToDelete = findIndexOfItemFromId(state.books, payload.id);
        return {
          books: [].concat(_toConsumableArray(state.books.slice(0, bookIndexToDelete)), _toConsumableArray(state.books.slice(bookIndexToDelete + 1)))
        };
      }
    case 'UPDATE_BOOK':
      {
        var bookIndexToUpdate = findIndexOfItemFromId(state.books, payload.id);
        var updatedBook = _extends({}, state.books[bookIndexToUpdate], { title: payload.title });
        return {
          books: [].concat(_toConsumableArray(state.books.slice(0, bookIndexToUpdate)), [updatedBook], _toConsumableArray(state.books.slice(bookIndexToUpdate + 1)))
        };
      }
    default:
      {
...
exports.default = function () {
  var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { cart: [], totalQty: 0, totalAmount: 0 };
  var action = arguments[1];
  ...
  switch (action.type) {
    ...
    case 'ADD_TO_CART_SUCCESS':
      {
        var updatedCart = action.payload;
        return {
          cart: updatedCart,
    ...
    case 'REMOVE_ITEM':
      {
        var itemIndex = findIndexOfItemFromId(state.cart, action.payload);
        var _updatedCart = [].concat(_toConsumableArray(state.cart.slice(0, itemIndex)), _toConsumableArray(state.cart.slice(itemIndex + 1)));
        return {
          cart: _updatedCart,
          totalAmount: totals(_updatedCart).amount,
          totalQty: totals(_updatedCart).qty
        };
      }
    default:
      return state;
...
function totals(payloadArr) {
  ...
  var totalQty = payloadArr.map(function (item) {
